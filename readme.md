...
插件不会接管用户手动打开的空白页或 Pixiv 标签页。运行时会创建并复用一个专用页面，配置重载时重新认领该页面，关闭插件或关闭 `ensureLogin` 后自动移除请求监听器并关闭页面。

## 开发与验证

当前项目以 `lib/index.js` 作为唯一运行源码和 npm 入口，不需要额外生成构建产物。类型声明位于 `lib/index.d.ts`，修改公共配置或导出时应同步维护。
...
`build` 会依次执行入口语法检查和不依赖真实 Pixiv 网络的回归测试。
