...
**现象**：将多张 `file://` 临时图片与作品信息合并为一条 `send_group_msg` 时，部分 OneBot 实现会返回 `retcode: 1200`，导致整条消息丢失。

**修复**：图片改为逐张独立发送。本地文件发送失败时自动回退为 data URI；无论图片发送是否失败，作品信息都会单独发送。临时图片会在发送完成后清理，并依据实际图片格式使用正确扩展名。
