...
function getImageFileInfo(pic) {
  if (pic.length >= 8 && pic.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))) {
    return { extension: "png", mimeType: "image/png" };
  }
  if (pic.length >= 3 && pic[0] === 255 && pic[1] === 216 && pic[2] === 255) {
    return { extension: "jpg", mimeType: "image/jpeg" };
  }
  if (pic.length >= 6 && (pic.subarray(0, 6).toString() === "GIF87a" || pic.subarray(0, 6).toString() === "GIF89a")) {
    return { extension: "gif", mimeType: "image/gif" };
  }
  if (pic.length >= 12 && pic.subarray(0, 4).toString() === "RIFF" && pic.subarray(8, 12).toString() === "WEBP") {
    return { extension: "webp", mimeType: "image/webp" };
  }
  return { extension: "jpg", mimeType: "image/jpeg" };
}
__name(getImageFileInfo, "getImageFileInfo");
function wait(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
__name(wait, "wait");
async function sendPixivImage(session, pic, index) {
  const logger2 = index_default.baseData.getLogger();
  const imageInfo = getImageFileInfo(pic);
  const tmpPath = import_path.default.join(import_os.default.tmpdir(), `pixiv_${Date.now()}_${Math.random().toString(36).slice(2)}.${imageInfo.extension}`);
  await import_fs.promises.writeFile(tmpPath, pic);
  try {
    try {
      await session.send(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", { src: (0, import_url.pathToFileURL)(tmpPath).href }));
      return;
    } catch (fileError) {
      logger2.warn(`第${index + 1}张图片本地文件发送失败，尝试 data URI: ${fileError?.message || fileError}`);
    }
    await session.send(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", { src: `data:${imageInfo.mimeType};base64,${pic.toString("base64")}` }));
  } finally {
    try {
      await import_fs.promises.unlink(tmpPath);
    } catch (error) {
      if (error?.code !== "ENOENT") logger2.warn(`清理临时图片失败: ${error?.message || error}`);
    }
  }
}
__name(sendPixivImage, "sendPixivImage");
async function randomTJPic(av, ms, isR18 = false) {
  const logger2 = index_default.baseData.getLogger();
  const ss = av.session;
  logger2.info(isR18 ? "正在随机获取R18图片" : "正在随机获取图片");
  const loadingMessage = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("message", { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("quote", { id: ss.messageId }),
    "你在此地不要走动"
  ] });
  try {
    try {
      await ss.send(loadingMessage);
    } catch (error) {
      logger2.warn(`发送加载提示失败，继续获取图片: ${error?.message || error}`);
    }
    ...
    for (const [index, pic] of (picBuffers || []).entries()) {
      ...
      try {
        await sendPixivImage(ss, compressed, index);
      } catch (error) {
        failedImages.push(index + 1);
        logger2.warn(`第${index + 1}张图片发送失败: ${error?.message || error}`);
      }
      if (index < picBuffers.length - 1) await wait(500);
    }
    const failureNotice = failedImages.length ? `\n\n${failedImages.length} 张图片发送失败（第 ${failedImages.join(", ")} 张），请查看控制台日志。` : "";
    await ss.send(`${artInfo || "未能获取作品信息"}${failureNotice}`);
  } catch (e) {
    logger2.warn(`获取失败，失败信息: ${e}`);
    try {
      await ss.send("获取失败，请联系管理员查看控制台");
    } catch (sendError) {
      logger2.warn(`发送失败提示时出错: ${sendError?.message || sendError}`);
    }
  }
}
